...
#include "ui_dialogthrusterdiagiii.h"

DialogthrusterdiagIII::DialogthrusterdiagIII(QWidget *parent)
    : QDialog(parent),
      ui(new Ui::DialogthrusterdiagIII),
      STBD_position({3.5, 3.7}),
      dataSpiral_tunnelcolor_STBD(arraylength_azimuth_third),
      dataSpiral_tunnelnocolor_STBD(arraylength_azimuth_third),
      dataSpiral_ball_STBD(arraylength_azimuth_third),
      PORT_position({3.5, 2.3}),
      dataSpiral_tunnelcolor_PORT(arraylength_azimuth_third),
      dataSpiral_tunnelnocolor_PORT(arraylength_azimuth_third),
      dataSpiral_ball_PORT(arraylength_azimuth_third),
...
DialogthrusterdiagIII::~DialogthrusterdiagIII() { delete ui; }

void DialogthrusterdiagIII::initializeAllUI(QCustomPlot *customPlot) {
...
void DialogthrusterdiagIII::initializevesselshape(
...
//
void DialogthrusterdiagIII::initializeSTBDProgressBar(QCustomPlot *customPlot) {
  ...
  for (unsigned i = 0; i != arraylength_outercircle; ++i) {
    ...
    dataSpiral_outercircle[i] = QCPCurveData(
        i, STBD_position[0] + azimuth_radius_third * std::cos(theta),
        STBD_position[1] + azimuth_radius_third * std::sin(theta));
  }
  ...
  int num_circle = arraylength_azimuth_third - 3;
  for (unsigned i = 0; i != num_circle; ++i) {
    double theta =
        i * tunnel_angle_third / (num_circle - 1) - tunnel_angle_third / 2;
    double cvalue = azimuth_radius_third * std::cos(theta);
    double svalue = azimuth_radius_third * std::sin(theta);
  ...
  dataSpiral_tunnelcolor_STBD[num_circle] =
      QCPCurveData(num_circle, STBD_position[0] + xvalue,
                   STBD_position[1] +
                       azimuth_radius_third * std::sin(tunnel_angle_third / 2));
  ...
  dataSpiral_tunnelcolor_STBD[num_circle + 2] = QCPCurveData(
      num_circle + 2,
      STBD_position[0] +
          azimuth_radius_third * std::cos(-tunnel_angle_third / 2),
      STBD_position[1] +
          azimuth_radius_third * std::sin(-tunnel_angle_third / 2));
  ...
  for (unsigned i = 0; i != arraylength_azimuth_third; ++i) {
    double theta = 2 * i * M_PI / (arraylength_azimuth_third - 1);
    dataSpiral_ball_STBD[i] =
        QCPCurveData(i, STBD_position[0] + azimuth_radius_third +
                            ball_radius_third * std::cos(theta),
                     STBD_position[1] + ball_radius_third * std::sin(theta));
  }
  ...
  angleText->position->setCoords(STBD_position[0] - 4.5 * azimuth_radius_third,
                                 STBD_position[1] + 0.7 * azimuth_radius_third);
  ...
  STBD_angleText->position->setCoords(
      STBD_position[0] - 2 * azimuth_radius_third,
      STBD_position[1] + 0.7 * azimuth_radius_third);
  ...
  RPMText->position->setCoords(STBD_position[0] - 4.5 * azimuth_radius_third,
                               STBD_position[1] - 0.5 * azimuth_radius_third);
  RPMText->setText("RPM");
...
//
void DialogthrusterdiagIII::initializePORTProgressBar(QCustomPlot *customPlot) {
  // generate data for outer circle
  const int arraylength_outercircle = 50;
  QVector<QCPCurveData> dataSpiral_outercircle(arraylength_outercircle);
  ...
  for (unsigned i = 0; i != arraylength_outercircle; ++i) {
    double theta = 2 * i * M_PI / (arraylength_outercircle - 1);
    dataSpiral_outercircle[i] = QCPCurveData(
        i, PORT_position[0] + azimuth_radius_third * std::cos(theta),
        PORT_position[1] + azimuth_radius_third * std::sin(theta));
  }
  ...
  for (unsigned i = 0; i != arraylength_azimuth_third; ++i) {
    ...
    dataSpiral_ball_PORT[i] =
        QCPCurveData(i, PORT_position[0] + azimuth_radius_third +
                            ball_radius_third * std::cos(theta),
                     PORT_position[1] + ball_radius_third * std::sin(theta));
  }
  ...
  // create static text for angle
  QCPItemText *angleText = new QCPItemText(customPlot);
  angleText->position->setCoords(PORT_position[0] - 4.5 * azimuth_radius_third,
                                 PORT_position[1] + 0.7 * azimuth_radius_third);
  ...
  PORT_angleText->position->setCoords(
      PORT_position[0] - 2 * azimuth_radius_third,
      PORT_position[1] + 0.7 * azimuth_radius_third);
  ...
  // create static text for RPM
  QCPItemText *RPMText = new QCPItemText(customPlot);
  RPMText->position->setCoords(PORT_position[0] - 4.5 * azimuth_radius_third,
                               PORT_position[1] - 0.5 * azimuth_radius_third);
  RPMText->setText("RPM");
  RPMText->setFont(QFont("SansSerif", 10));
  RPMText->setPositionAlignment(Qt::AlignBottom | Qt::AlignLeft);
  RPMText->setColor(Qt::black);
...
void DialogthrusterdiagIII::initializeTunnelProgressBar_first(
  ...
  double half_length = tunnel_length_third / 2;
  double half_width = tunnel_width_third / 2;
  ...
  dataSpiral_tunnel_first[0] =
      QCPCurveData(0, tunnel_first_position[0] + tunnel_width_third / 2,
                   tunnel_first_position[1]);
  dataSpiral_tunnel_first[1] =
      QCPCurveData(1, tunnel_first_position[0] + tunnel_width_third / 2,
                   tunnel_first_position[1] + half_length);
  dataSpiral_tunnel_first[2] =
      QCPCurveData(2, tunnel_first_position[0] - tunnel_width_third / 2,
                   tunnel_first_position[1] + half_length);
  dataSpiral_tunnel_first[3] =
      QCPCurveData(3, tunnel_first_position[0] - tunnel_width_third / 2,
                   tunnel_first_position[1]);
  dataSpiral_tunnel_first[4] =
      QCPCurveData(4, tunnel_first_position[0] + tunnel_width_third / 2,
                   tunnel_first_position[1]);
  ...
  tunnel_first_RPMText->position->setCoords(
      ...
      tunnel_first_position[1] + 0.8 * tunnel_length_third);
...
void DialogthrusterdiagIII::updateTunnelProgressBar_first(int _rpm) {
  // generate data for progress data
  double progress_y =
      tunnel_length_third * _rpm / max_tunnel_rotation_third / 2;
  double half_width = tunnel_width_third / 2;
...
void DialogthrusterdiagIII::updateTunnelProgressBar_second(int _rpm) {
  // generate data for progress data
  double progress_y =
      tunnel_length_third * _rpm / max_tunnel_rotation_third / 2;
  double half_width = tunnel_width_third / 2;
...
void DialogthrusterdiagIII::initializeTunnelProgressBar_second(
    QCustomPlot *customPlot) {
  // generate data for outer square
  const int arraylength_outersquare = 5;
  QVector<QCPCurveData> dataSpiral_outersquare(arraylength_outersquare);
  ...
  tunnel_second_RPMText->position->setCoords(
      ...
      tunnel_second_position[1] + 0.8 * tunnel_length_third);
...
void DialogthrusterdiagIII::updateSTBDProgressBar(double _percent,
                                                  double _orientation) {
  // generate data for circle
  double half_angle = tunnel_angle_third / 2;
  int num_circle = arraylength_azimuth_third - 3;
  for (unsigned i = 0; i != num_circle; ++i) {
    double theta =
        _orientation + i * tunnel_angle_third / (num_circle - 1) - half_angle;
    double cvalue = azimuth_radius_third * std::cos(theta);
    double svalue = azimuth_radius_third * std::sin(theta);
    dataSpiral_tunnelcolor_STBD[i] =
        QCPCurveData(i, STBD_position[0] + cvalue, STBD_position[1] + svalue);
  ...
  double length = _percent * 2 * azimuth_radius_third * std::cos(half_angle);
  double Xb = azimuth_radius_third * std::cos(_orientation - half_angle);
  double Yb = azimuth_radius_third * std::sin(_orientation - half_angle);
  double X_pa = azimuth_radius_third * std::cos(_orientation + half_angle) -
                length * std::cos(_orientation);
  double Y_pa = azimuth_radius_third * std::sin(_orientation + half_angle) -
  ...
  // generate data for ball
  double ball_cvalue =
      STBD_position[0] + azimuth_radius_third * std::cos(_orientation);
  double ball_svalue =
      STBD_position[1] + azimuth_radius_third * std::sin(_orientation);
  for (unsigned i = 0; i != arraylength_azimuth_third; ++i) {
    double theta = 2 * i * M_PI / (arraylength_azimuth_third - 1);
    dataSpiral_ball_STBD[i] =
        QCPCurveData(i, ball_cvalue + ball_radius_third * std::cos(theta),
                     ball_svalue + ball_radius_third * std::sin(theta));
  }
...
void DialogthrusterdiagIII::updatePORTProgressBar(double _percent,
                                                  double _orientation) {
  // generate data for circle
  double half_angle = tunnel_angle_third / 2;
  int num_circle = arraylength_azimuth_third - 3;
  ...
  // generate data for ball
  double ball_cvalue =
      PORT_position[0] + azimuth_radius_third * std::cos(_orientation);
  double ball_svalue =
      PORT_position[1] + azimuth_radius_third * std::sin(_orientation);
  for (unsigned i = 0; i != arraylength_azimuth_third; ++i) {
    double theta = 2 * i * M_PI / (arraylength_azimuth_third - 1);
    dataSpiral_ball_PORT[i] =
        QCPCurveData(i, ball_cvalue + ball_radius_third * std::cos(theta),
...
void DialogthrusterdiagIII::updateSTBDText(int _rpm, int _orientation) {
  STBD_angleText->setText(QString::number(_orientation));
  STBD_RPMText->setText(QString::number(_rpm));
}

void DialogthrusterdiagIII::updatePORTText(int _rpm, int _orientation) {
  PORT_angleText->setText(QString::number(_orientation));
  PORT_RPMText->setText(QString::number(_rpm));
}

void DialogthrusterdiagIII::setupthrusterRealtimeData() {
...
void DialogthrusterdiagIII::AzimuthDataSlot() {
  ...
  updateSTBDProgressBar((double)rpm_star / max_azimuth_rotation_third,
                        -angle_star * M_PI / 180);
  updatePORTProgressBar((double)rpm_port / max_azimuth_rotation_third,
                        -angle_port * M_PI / 180);
